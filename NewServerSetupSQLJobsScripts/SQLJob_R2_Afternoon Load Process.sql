...
/****** Object:  Job [13:20 R2_Afternoon Load Process]    Script Date: 9/12/2018 7:20:32 AM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [R2_Daily_Job]    Script Date: 9/12/2018 7:20:32 AM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'13:20 R2_Afternoon Load Process', 
		@enabled=0, 
...
9. Send Failure Notification - only if job fails', 
		@category_name=N'R2_Daily_Job', 
		@owner_login_name=N'sa', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Move Data to ResourceDetails]    Script Date: 9/12/2018 7:20:32 AM ******/
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Run ResourceCleanUpData]    Script Date: 9/12/2018 7:20:32 AM ******/
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Move Manual Hours to ResourceDetails]    Script Date: 9/12/2018 7:20:32 AM ******/
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Insert Workdate for Billing Month into ResourceWorkdate Table]    Script Date: 9/12/2018 7:20:32 AM ******/
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [PreProcess Data]    Script Date: 9/12/2018 7:20:32 AM ******/
...
if exists ( select * from R2_Import..Log where TS > dateadd(hh, -1, getdate()) and Text like ''%Ended moving manual hours%'' )
	...
	exec msdb.dbo.sp_send_dbmail @profile_name = ''SQL_DBMail'', @recipients = ''lizzy.lopez@dxc.com'', @subject = ''R2 131C PreProcess_All_Tables Failed!'', @body = ''The PreProcess_All_Tables process failed!''
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Process Data for Reports Database]    Script Date: 9/12/2018 7:20:32 AM ******/
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Prebuild Reports]    Script Date: 9/12/2018 7:20:33 AM ******/
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Send Load Notification]    Script Date: 9/12/2018 7:20:33 AM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Send Load Notification', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'exec Send_Notification @AM_PM_Indicator = ''PM''
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Send Failure Notificaiton]    Script Date: 9/12/2018 7:20:33 AM ******/
...
exec msdb.dbo.sp_send_dbmail 
                ...
                @subject = ''R2 PM Load Failed'', 
             	@body = ''The job that runs the afternoon load failed in 131C.'', 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'R2 Afternoon Load', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
		...
		@active_start_date=20150626, 
		@active_end_date=99991231, 
		@active_start_time=132000, 
		@active_end_time=235959, 
		@schedule_uid=N'c4025ac1-17e0-49a2-8d5a-95af0e86a33f'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
