...
select ProgramGroup,Program,AFEDesc,ProjectTitle,TaskName,EventList,eventname,ResourceName, billingid,Prog_GroupID,ProgramID,AFE_DescID,Client,BillingFlag,ClientFundingPct, ClientFundedBy,BillingType,Type,TaskClientFundingPct,TaskClientFundedBy,Funding_CatID,ProjectStatusID, ResourceNumber,ProjectID,TaskID,eventlistid,sum(hours) as hours, COBusinessLead
into #TEMP_IN from dbo.SP_Get_AFE_Resource_Detail
group by ProgramGroup,Program,AFEDesc,ProjectTitle,TaskName,EventList,eventname,ResourceName, billingid,Prog_GroupID,ProgramID,AFE_DescID,Client,BillingFlag,ClientFundingPct, ClientFundedBy,BillingType,Type,TaskClientFundingPct,TaskClientFundedBy, Funding_CatID,ProjectStatusID,ResourceNumber,ProjectID,TaskID,eventlistid, COBusinessLead
...
update #TEMP_IN set Hours = isnull(ClientFundingPct,100)100Hours where isnull(ClientFundingPct,0)  0
update #TEMP_IN set Hours = isnull(TaskClientFundingPct,100)100Hours where isnull(TaskClientFundingPct,0)  0
...
-- Declare variables.
--DECLARE @CurProgramGroup varchar(100), @CurProg_GroupID int, @CurProgram varchar(100), @CurProgramID  int, @CurCOBusinessLead varchar (100), @CurAFEDesc varchar (100), @CurAFE_DescID int, @CurProjectTitle varchar(100), 
DECLARE @CurProgramGroup varchar(100), @CurProg_GroupID varchar(100), @CurProgram varchar(100), @CurProgramID varchar(100), @CurCOBusinessLead varchar (100), @CurAFEDesc varchar (100), @CurAFE_DescID varchar(100), @CurProjectTitle varchar(100), 
@CurProjID varchar(100), @CurTaskName varchar(100), @CurTaskID varchar(100), @CurResourceName varchar(100), @CurEventList varchar(100), @CurEventName varchar(100), @Billing_Type varchar(150), @Hours decimal(10,2), @MaxProgGroup int, @MaxProg int, @MaxAFEDesc int, @MaxProj bigint, @MaxTask int, @MaxEvent int
...
BEGIN
	...
	select 10, 'ProgGroupTotal', @CurProgramGroup, @CurProg_GroupID
            ...
            BEGIN
                ...
                BEGIN
					...
					DECLARE Event_cursor CURSOR FOR
					    ...
					    where ProgramGroup = @CurProgramGroup and Program = @CurProgram and AFE_DescID = @CurAFE_DescID and ProjectID = @CurProjID and TaskID = @CurTaskID 
						--and EventList is not NULL and EventName is not NULL 
					...
					BEGIN
						...
						DECLARE Resource_cursor CURSOR FOR
							select  ResourceName,BillingType, Hours from #TEMP_IN
						    where ProgramGroup = @CurProgramGroup and Program = @CurProgram and AFE_DescID = @CurAFE_DescID and ProjectID = @CurProjID and TaskID = @CurTaskID 
								--and EventList = @CurEventList and EventName = @CurEventName 
								and ResourceName is not null
	...
	-----------------------------------------------------------------------------------------------------------------------
        ...
        update #TEMP_OUT set ActualFTEs = TotalHours143.5 where AutoKey  @MaxAFEDesc and RecNumber in (40, 50, 56, 60)
		...
		update #TEMP_OUT set ActualFTEs = TotalHours143.5 where AutoKey = @MaxAFEDesc
    ...
    set JuniorSEHours = @JuniorSEHours,
        ...
        PLTPFHours = @PLTPFHours,
        TotalHours = @TotalHours,
        ActualFTEs = @TotalHours143.5,
        COApprovedFTEs = @COApprovedFTEs,
        EDSVariance = @TotalHours143.5 - @COApprovedFTEs
...
update #TEMP_OUT
set JuniorSEHours = @JuniorSEHours143.5,
    SEHours = @SEHours143.5,
    ADVSEHours = @ADVSEHours143.5,
    SeniorSEHours = @SeniorSEHours143.5,
    PLHours = @PLHours143.5,
    JuniorTPFHours = @JuniorTPFHours143.5,
    SETPFHours = @SETPFHours143.5,
    ADVTPFHours = @ADVTPFHours143.5,
    SeniorTPFHours = @SeniorTPFHours143.5,
    PLTPFHours = @PLTPFHours143.5,
    TotalHours = @TotalHours143.5, -- value 1
    ActualFTEs = @TotalHours143.5, -- value 1
    COApprovedFTEs = @COApprovedFTEs
where RecNumber = 3

update #TEMP_OUT set ActualFTEs = @TotalHours143.5 where RecNumber = 0
