...
DECLARE @CurProgramGroup varchar(100), @CurProgramGroupTotal decimal(10,2), @CurProg_GroupID int, @CurProgram varchar(100), @CurCOBusinessLead varchar(100), @CurProgramID varchar(100), @CurAFEDesc varchar (100), @CurAFE_DescID int, @CurTaskCatCode varchar(100), @CurTaskName varchar(100), @CurTaskID varchar(100), @CurResource varchar(100), @CurEDSNETID varchar(10)
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				--------------------------------------------------------------------------------
				-- Open the Task  Cursor		
				--------------------------------------------------------------------------------
				DECLARE Task_cursor CURSOR FOR 
   					select distinct TaskName, TaskID from #TEMP_IN
					where Prog_GroupID = @CurProg_GroupID and ProgramID = @CurProgramID and AFE_DescID = @CurAFE_DescID and TaskCategorycode = @CurTaskCatCode order by TaskName
				OPEN Task_cursor
				FETCH NEXT FROM Task_cursor INTO @CurTaskName, @CurTaskID
				WHILE @@FETCH_STATUS = 0
				BEGIN
		  		    insert #TEMP_OUT (RecNumber, RecType, TaskCategoryRecNum50,TaskCategory, RecTypeID,     TaskID,Prog_GroupID, ProgramID)
					select 50, 'Task Name ',@CurTaskCatCode, @CurTaskName , @CurAFE_DescID,  @CurTaskID, @CurProg_GroupID, @CurProgramID

					--------------------------------------------------------------------------------
					-- Open the RESOURCE  Cursor		
					--------------------------------------------------------------------------------
					DECLARE Resource_cursor CURSOR FOR 
   						select distinct Resource, EDSNETID from #TEMP_IN
						where Prog_GroupID = @CurProg_GroupID and ProgramID = @CurProgramID and AFE_DescID = @CurAFE_DescID and TaskCategorycode = @CurTaskCatCode and TaskID = @CurTaskID order by Resource
					OPEN Resource_cursor
					FETCH NEXT FROM Resource_cursor INTO @CurResource, @CurEDSNETID
					WHILE @@FETCH_STATUS = 0
					BEGIN
			  		    insert #TEMP_OUT (RecNumber, RecType,  TaskCategory, ResourceNumber, TaskID)
						select 60, 'Resource Name ',@CurResource ,  @CurEDSNETID, @CurTaskID

						-----------------------------------------------------------------------------------
						-- sum up the ONSHORE months for this RESOURCE build out the 12 month buckets	---
						-----------------------------------------------------------------------------------
						DECLARE Month_cursor_onshore CURSOR FOR 
							select YYYY_Year,MM_Month, sum(hours) as TotalHours from #TEMP_IN
      						where Prog_GroupID = @CurProg_GroupID and ProgramID = @CurProgramID and AFE_DescID = @CurAFE_DescID and TaskCategoryCode = @CurTaskCatCode and TaskID = @CurTaskID and EDSNETID = @CurEDSNETID 
      						and NewBillingType <> 'FTP Staff Aug' and Onshore = 1 group by YYYY_Year, MM_Month
						...
						-----------------------------------------------------------------------------------
						-- sum up the FTP ONSHORE months for this RESOURCE build out the 12 month buckets	---
						-----------------------------------------------------------------------------------
						DECLARE Month_cursor_FTPonshore CURSOR FOR 
							select YYYY_Year,MM_Month, sum(hours) as TotalHours from #TEMP_IN
      						where Prog_GroupID = @CurProg_GroupID and ProgramID = @CurProgramID and AFE_DescID = @CurAFE_DescID and TaskCategoryCode = @CurTaskCatCode and TaskID = @CurTaskID and EDSNETID = @CurEDSNETID 
						...
						-----------------------------------------------------------------------------------
						-- sum up the OFFSHORE months for this RESOURCE build out the 12 month buckets	---
						-----------------------------------------------------------------------------------
						DECLARE Month_cursor_offshore CURSOR FOR 
							select YYYY_Year,MM_Month, sum(hours) as TotalHours from #TEMP_IN
      						where Prog_GroupID = @CurProg_GroupID and ProgramID = @CurProgramID and AFE_DescID = @CurAFE_DescID and TaskCategoryCode = @CurTaskCatCode and TaskID = @CurTaskID and EDSNETID = @CurEDSNETID 
      						and NewBillingType <> 'FTP Staff Aug' and Offshore = 1 group by YYYY_Year, MM_Month
						...
						-----------------------------------------------------------------------------------
						-- sum up FTP OFFSHORE months for this RESOURCE build out the 12 month buckets	---
						-----------------------------------------------------------------------------------
						DECLARE Month_cursor_FTPoffshore CURSOR FOR 
							select YYYY_Year,MM_Month, sum(hours) as TotalHours from #TEMP_IN
      						where Prog_GroupID = @CurProg_GroupID and ProgramID = @CurProgramID and AFE_DescID = @CurAFE_DescID and TaskCategoryCode = @CurTaskCatCode and TaskID = @CurTaskID and EDSNETID = @CurEDSNETID 
						...
						---------------------------------------------------------------------------------------------------
						-- Update the TEMP_OUT table RESOURCE Record containing all of the 12 buckets totals
						---------------------------------------------------------------------------------------------------
						update #TEMP_OUT set
							Bucket1_Hours = isnull(@BucketOnshore1,0) + isnull(@BucketFTPOnshore1,0) + isnull(@BucketOffshore1,0) + isnull(@BucketFTPOffshore1,0), Bucket1_FTEs = isnull(@BucketOnshore1,0)/@FTEFactorOnshore + isnull(@BucketFTPOnshore1,0)/@FTPBillingRate + isnull(@BucketOffshore1,0)/@FTEFactorOffshore + isnull(@BucketFTPOffshore1,0)/@FTPBillingRate, Bucket1_FTEs_hover = isnull(@BucketOnshore1,0)/@FTEFactorOnshore + isnull(@BucketFTPOnshore1,0)/@FTPBillingRate + isnull(@BucketOffshore1,0)/@FTEFactorOffshore + isnull(@BucketFTPOffshore1,0)/@FTPBillingRate,	 		
							Bucket2_Hours = isnull(@BucketOnshore2,0) + isnull(@BucketFTPOnshore2,0) + isnull(@BucketOffshore2,0) + isnull(@BucketFTPOffshore2,0), Bucket2_FTEs = isnull(@BucketOnshore2,0)/@FTEFactorOnshore + isnull(@BucketFTPOnshore2,0)/@FTPBillingRate + isnull(@BucketOffshore2,0)/@FTEFactorOffshore + isnull(@BucketFTPOffshore2,0)/@FTPBillingRate, Bucket2_FTEs_hover = isnull(@BucketOnshore2,0)/@FTEFactorOnshore + isnull(@BucketFTPOnshore2,0)/@FTPBillingRate + isnull(@BucketOffshore2,0)/@FTEFactorOffshore + isnull(@BucketFTPOffshore2,0)/@FTPBillingRate,	 		
							...
							TotalFTEs_hover = ((isnull(@BucketOnshore1,0))+(isnull(@BucketOnshore2,0))+(isnull(@BucketOnshore3,0))+(isnull(@BucketOnshore4,0))+(isnull(@BucketOnshore5,0))+(isnull(@BucketOnshore6,0))+(isnull(@BucketOnshore7,0))+(isnull(@BucketOnshore8,0))+(isnull(@BucketOnshore9,0))+(isnull(@BucketOnshore10,0))+(isnull(@BucketOnshore11,0))+(isnull(@BucketOnshore12,0)))/@FTEFactorOnshore + ((isnull(@BucketFTPOnshore1,0))+(isnull(@BucketFTPOnshore2,0))+(isnull(@BucketFTPOnshore3,0))+(isnull(@BucketFTPOnshore4,0))+(isnull(@BucketFTPOnshore5,0))+(isnull(@BucketFTPOnshore6,0))+(isnull(@BucketFTPOnshore7,0))+(isnull(@BucketFTPOnshore8,0))+(isnull(@BucketFTPOnshore9,0))+(isnull(@BucketFTPOnshore10,0))+(isnull(@BucketFTPOnshore11,0))+(isnull(@BucketFTPOnshore12,0)))/@FTPBillingRate +	((isnull(@BucketOffshore1,0))+(isnull(@BucketOffshore2,0))+(isnull(@BucketOffshore3,0))+(isnull(@BucketOffshore4,0))+(isnull(@BucketOffshore5,0))+(isnull(@BucketOffshore6,0))+(isnull(@BucketOffshore7,0))+(isnull(@BucketOffshore8,0))+(isnull(@BucketOffshore9,0))+(isnull(@BucketOffshore10,0))+(isnull(@BucketOffshore11,0))+(isnull(@BucketOffshore12,0)))/@FTEFactorOffshore + ((isnull(@BucketFTPOffshore1,0))+(isnull(@BucketFTPOffshore2,0))+(isnull(@BucketFTPOffshore3,0))+(isnull(@BucketFTPOffshore4,0))+(isnull(@BucketFTPOffshore5,0))+(isnull(@BucketFTPOffshore6,0))+(isnull(@BucketFTPOffshore7,0))+(isnull(@BucketFTPOffshore8,0))+(isnull(@BucketFTPOffshore9,0))+(isnull(@BucketFTPOffshore10,0))+(isnull(@BucketFTPOffshore11,0))+(isnull(@BucketFTPOffshore12,0)))/@FTPBillingRate
						where RecNumber = 60   and ResourceNumber = @CurEDSNETID and TaskID = @CurTaskID
					...
					---------------------------------------------------------------------------------------------------
					-- Sum all Resource Records for this TASK (type = 60) with current TASK ID 
					-- Then Update #TEMP_OUT with the TASK Totals Record Type = 50 by Month
					---------------------------------------------------------------------------------------------------
					DECLARE @TotalTask_Bucket1 decimal(12,2), @TotalTask_Bucket2 decimal(12,2), @TotalTask_Bucket3 decimal(12,2), @TotalTask_Bucket4 decimal(12,2),@TotalTask_Bucket5 decimal(12,2), @TotalTask_Bucket6 decimal(12,2), @TotalTask_Bucket7 decimal(12,2), @TotalTask_Bucket8 decimal(12,2),@TotalTask_Bucket9 decimal(12,2), @TotalTask_bucket10 decimal(12,2), @TotalTask_bucket11 decimal(12,2),@TotalTask_bucket12 decimal(12,2)
					select @TotalTask_Bucket1 = sum(isnull(Bucket1_Hours,0)), @TotalTask_Bucket2 = sum(isnull(Bucket2_hours,0)), @TotalTask_Bucket3 = sum(isnull(Bucket3_hours,0)), @TotalTask_Bucket4 = sum(isnull(Bucket4_hours,0)), @TotalTask_Bucket5 = sum(isnull(Bucket5_hours,0)), @TotalTask_Bucket6 = sum(isnull(Bucket6_hours,0)), @TotalTask_Bucket7 = sum(isnull(Bucket7_hours,0)), @TotalTask_Bucket8 = sum(isnull(Bucket8_hours,0)), @TotalTask_Bucket9 = sum(isnull(Bucket9_hours,0)), @TotalTask_bucket10 = sum(isnull(bucket10_hours,0)), @TotalTask_bucket11 = sum(isnull(bucket11_hours,0)), @TotalTask_bucket12 = sum(isnull(bucket12_hours,0))
					from #TEMP_OUT where RecNumber = 60 and TaskID = @CurTaskID 
								
					declare @TotalTask_FTE1 decimal(5,2), @TotalTask_FTE2 decimal(5,2), @TotalTask_FTE3 decimal(5,2), @TotalTask_FTE4 decimal(5,2), @TotalTask_FTE5 decimal(5,2), @TotalTask_FTE6 decimal(5,2), @TotalTask_FTE7 decimal(5,2), @TotalTask_FTE8 decimal(5,2), @TotalTask_FTE9 decimal(5,2), @TotalTask_FTE10 decimal(5,2), @TotalTask_FTE11 decimal(5,2), @TotalTask_FTE12 decimal(5,2)
					select @TotalTask_FTE1 = sum(isnull(Bucket1_FTEs,0)), @TotalTask_FTE2 = sum(isnull(Bucket2_FTEs,0)), @TotalTask_FTE3 = sum(isnull(Bucket3_FTEs,0)), @TotalTask_FTE4 = sum(isnull(Bucket4_FTEs,0)), @TotalTask_FTE5 = sum(isnull(Bucket5_FTEs,0)), @TotalTask_FTE6 = sum(isnull(Bucket6_FTEs,0)), @TotalTask_FTE7 = sum(isnull(Bucket7_FTEs,0)), @TotalTask_FTE8 = sum(isnull(Bucket8_FTEs,0)), @TotalTask_FTE9 = sum(isnull(Bucket9_FTEs,0)), @TotalTask_FTE10 = sum(isnull(Bucket10_FTEs,0)), @TotalTask_FTE11 = sum(isnull(Bucket11_FTEs,0)), @TotalTask_FTE12 = sum(isnull(Bucket12_FTEs,0))
					from #TEMP_OUT where RecNumber = 60 and TaskID = @CurTaskID 
					
					declare @TotalTask_FTE1_hover decimal(15,7), @TotalTask_FTE2_hover decimal(15,7), @TotalTask_FTE3_hover decimal(15,7), @TotalTask_FTE4_hover decimal(15,7), @TotalTask_FTE5_hover decimal(15,7), @TotalTask_FTE6_hover decimal(15,7), @TotalTask_FTE7_hover decimal(15,7), @TotalTask_FTE8_hover decimal(15,7), @TotalTask_FTE9_hover decimal(15,7), @TotalTask_FTE10_hover decimal(15,7), @TotalTask_FTE11_hover decimal(15,7), @TotalTask_FTE12_hover decimal(15,7)
					select @TotalTask_FTE1_hover = sum(isnull(Bucket1_FTEs_hover,0)), @TotalTask_FTE2_hover = sum(isnull(Bucket2_FTEs_hover,0)), @TotalTask_FTE3_hover = sum(isnull(Bucket3_FTEs_hover,0)), @TotalTask_FTE4_hover = sum(isnull(Bucket4_FTEs_hover,0)), @TotalTask_FTE5_hover = sum(isnull(Bucket5_FTEs_hover,0)), @TotalTask_FTE6_hover = sum(isnull(Bucket6_FTEs_hover,0)), @TotalTask_FTE7_hover = sum(isnull(Bucket7_FTEs_hover,0)), @TotalTask_FTE8_hover = sum(isnull(Bucket8_FTEs_hover,0)), @TotalTask_FTE9_hover = sum(isnull(Bucket9_FTEs_hover,0)), @TotalTask_FTE10_hover = sum(isnull(Bucket10_FTEs_hover,0)), @TotalTask_FTE11_hover = sum(isnull(Bucket11_FTEs_hover,0)), @TotalTask_FTE12_hover = sum(isnull(Bucket12_FTEs_hover,0))
					from #TEMP_OUT where RecNumber = 60 and TaskID = @CurTaskID 

					-- Update TASK record  Rec Type = 50
					update #TEMP_OUT set
						Bucket1_Hours = isnull(@TotalTask_Bucket1,0),Bucket1_FTEs = isnull(@TotalTask_FTE1,0), Bucket1_FTEs_hover = isnull(@TotalTask_FTE1_hover,0),
						Bucket2_Hours = isnull(@TotalTask_Bucket2,0),Bucket2_FTEs = isnull(@TotalTask_FTE2,0), Bucket2_FTEs_hover = isnull(@TotalTask_FTE2_hover,0),
						Bucket3_Hours = isnull(@TotalTask_Bucket3,0),Bucket3_FTEs = isnull(@TotalTask_FTE3,0), Bucket3_FTEs_hover = isnull(@TotalTask_FTE3_hover,0),
						Bucket4_Hours = isnull(@TotalTask_Bucket4,0),Bucket4_FTEs = isnull(@TotalTask_FTE4,0), Bucket4_FTEs_hover = isnull(@TotalTask_FTE4_hover,0),
						Bucket5_Hours = isnull(@TotalTask_Bucket5,0),Bucket5_FTEs = isnull(@TotalTask_FTE5,0), Bucket5_FTEs_hover = isnull(@TotalTask_FTE5_hover,0),
						Bucket6_Hours = isnull(@TotalTask_Bucket6,0),Bucket6_FTEs = isnull(@TotalTask_FTE6,0), Bucket6_FTEs_hover = isnull(@TotalTask_FTE6_hover,0),
						Bucket7_Hours = isnull(@TotalTask_Bucket7,0),Bucket7_FTEs = isnull(@TotalTask_FTE7,0), Bucket7_FTEs_hover = isnull(@TotalTask_FTE7_hover,0),
						Bucket8_Hours = isnull(@TotalTask_Bucket8,0),Bucket8_FTEs = isnull(@TotalTask_FTE8,0), Bucket8_FTEs_hover = isnull(@TotalTask_FTE8_hover,0),
						Bucket9_Hours = isnull(@TotalTask_Bucket9,0),Bucket9_FTEs = isnull(@TotalTask_FTE9,0), Bucket9_FTEs_hover = isnull(@TotalTask_FTE9_hover,0),
						Bucket10_Hours = isnull(@TotalTask_Bucket10,0),Bucket10_FTEs = isnull(@TotalTask_FTE10,0), Bucket10_FTEs_hover = isnull(@TotalTask_FTE10_hover,0),
						Bucket11_Hours = isnull(@TotalTask_Bucket11,0),Bucket11_FTEs = isnull(@TotalTask_FTE11,0), Bucket11_FTEs_hover = isnull(@TotalTask_FTE11_hover,0),
						Bucket12_Hours = isnull(@TotalTask_Bucket12,0),Bucket12_FTEs = isnull(@TotalTask_FTE12,0), Bucket12_FTEs_hover = isnull(@TotalTask_FTE12_hover,0),
						TotalHours = (isnull(@TotalTask_Bucket1,0))+(isnull(@TotalTask_Bucket2,0))+(isnull(@TotalTask_Bucket3,0))+(isnull(@TotalTask_Bucket4,0))+(isnull(@TotalTask_Bucket5,0))+(isnull(@TotalTask_Bucket6,0))+(isnull(@TotalTask_Bucket7,0))+(isnull(@TotalTask_Bucket8,0))+(isnull(@TotalTask_Bucket9,0))+(isnull(@TotalTask_Bucket10,0))+(isnull(@TotalTask_Bucket11,0))+(isnull(@TotalTask_Bucket12,0)),
						TotalFTEs = ( (isnull(@TotalTask_FTE1,0))+(isnull(@TotalTask_FTE2,0))+(isnull(@TotalTask_FTE3,0))+(isnull(@TotalTask_FTE4,0))+(isnull(@TotalTask_FTE5,0))+(isnull(@TotalTask_FTE6,0))+(isnull(@TotalTask_FTE7,0))+(isnull(@TotalTask_FTE8,0))+(isnull(@TotalTask_FTE9,0))+(isnull(@TotalTask_FTE10,0))+(isnull(@TotalTask_FTE11,0))+(isnull(@TotalTask_FTE12,0)) ),
						TotalFTEs_hover = ( (isnull(@TotalTask_FTE1_hover,0))+(isnull(@TotalTask_FTE2_hover,0))+(isnull(@TotalTask_FTE3_hover,0))+(isnull(@TotalTask_FTE4_hover,0))+(isnull(@TotalTask_FTE5_hover,0))+(isnull(@TotalTask_FTE6_hover,0))+(isnull(@TotalTask_FTE7_hover,0))+(isnull(@TotalTask_FTE8_hover,0))+(isnull(@TotalTask_FTE9_hover,0))+(isnull(@TotalTask_FTE10_hover,0))+(isnull(@TotalTask_FTE11_hover,0))+(isnull(@TotalTask_FTE12_hover,0)) )
					where RecNumber = 50 and TaskID = @CurTaskID	
				...
				---------------------------------------------------------------------------------------------------
				-- Sum all Task  Records for this Task Category (type = 40) with current Task Category Code and AFE Desc ID
				-- Then Update #TEMP_OUT with the Task Category Totals Record Type = 40 by Month
				---------------------------------------------------------------------------------------------------
				DECLARE @TotalCat_Bucket1 decimal(12,2), @TotalCat_Bucket2 decimal(12,2), @TotalCat_Bucket3 decimal(12,2), @TotalCat_Bucket4 decimal(12,2),@TotalCat_Bucket5 decimal(12,2), @TotalCat_Bucket6 decimal(12,2), @TotalCat_Bucket7 decimal(12,2), @TotalCat_Bucket8 decimal(12,2),@TotalCat_Bucket9 decimal(12,2), @TotalCat_bucket10 decimal(12,2), @TotalCat_bucket11 decimal(12,2),@TotalCat_bucket12 decimal(12,2)
				select @TotalCat_Bucket1 = sum(isnull(Bucket1_Hours,0)), @TotalCat_Bucket2 = sum(isnull(Bucket2_hours,0)), @TotalCat_Bucket3 = sum(isnull(Bucket3_hours,0)), @TotalCat_Bucket4 = sum(isnull(Bucket4_hours,0)), @TotalCat_Bucket5 = sum(isnull(Bucket5_hours,0)), @TotalCat_Bucket6 = sum(isnull(Bucket6_hours,0)), @TotalCat_Bucket7 = sum(isnull(Bucket7_hours,0)), @TotalCat_Bucket8 = sum(isnull(Bucket8_hours,0)), @TotalCat_Bucket9 = sum(isnull(Bucket9_hours,0)), @TotalCat_bucket10 = sum(isnull(bucket10_hours,0)), @TotalCat_bucket11 = sum(isnull(bucket11_hours,0)), @TotalCat_bucket12 = sum(isnull(bucket12_hours,0))
				from #TEMP_OUT where RecNumber = 50 and TaskCategoryRecNum50 = @CurTaskCatCode and RecTypeID = @CurAFE_DescID 

				DECLARE @TotalCat_FTE1 decimal(12,2), @TotalCat_FTE2 decimal(12,2), @TotalCat_FTE3 decimal(12,2), @TotalCat_FTE4 decimal(12,2), @TotalCat_FTE5 decimal(12,2), @TotalCat_FTE6 decimal(12,2), @TotalCat_FTE7 decimal(12,2), @TotalCat_FTE8 decimal(12,2), @TotalCat_FTE9 decimal(12,2), @TotalCat_FTE10 decimal(12,2), @TotalCat_FTE11 decimal(12,2),@TotalCat_FTE12 decimal(12,2)
				select @TotalCat_FTE1 = sum(isnull(Bucket1_FTEs,0)), @TotalCat_FTE2 = sum(isnull(Bucket2_FTEs,0)), @TotalCat_FTE3 = sum(isnull(Bucket3_FTEs,0)), @TotalCat_FTE4 = sum(isnull(Bucket4_FTEs,0)), @TotalCat_FTE5 = sum(isnull(Bucket5_FTEs,0)), @TotalCat_FTE6 = sum(isnull(Bucket6_FTEs,0)), @TotalCat_FTE7 = sum(isnull(Bucket7_FTEs,0)), @TotalCat_FTE8 = sum(isnull(Bucket8_FTEs,0)), @TotalCat_FTE9 = sum(isnull(Bucket9_FTEs,0)), @TotalCat_FTE10 = sum(isnull(bucket10_FTEs,0)), @TotalCat_FTE11 = sum(isnull(bucket11_FTEs,0)), @TotalCat_FTE12 = sum(isnull(bucket12_FTEs,0))  
				from #TEMP_OUT where RecNumber = 50 and TaskCategoryRecNum50 = @CurTaskCatCode and RecTypeID = @CurAFE_DescID 

				DECLARE @TotalCat_FTE1_hover decimal(15,7), @TotalCat_FTE2_hover decimal(15,7), @TotalCat_FTE3_hover decimal(15,7), @TotalCat_FTE4_hover decimal(15,7), @TotalCat_FTE5_hover decimal(15,7), @TotalCat_FTE6_hover decimal(15,7), @TotalCat_FTE7_hover decimal(15,7), @TotalCat_FTE8_hover decimal(15,7), @TotalCat_FTE9_hover decimal(15,7), @TotalCat_FTE10_hover decimal(15,7), @TotalCat_FTE11_hover decimal(15,7),@TotalCat_FTE12_hover decimal(15,7)
				select @TotalCat_FTE1_hover = sum(isnull(Bucket1_FTEs_hover,0)), @TotalCat_FTE2_hover = sum(isnull(Bucket2_FTEs_hover,0)), @TotalCat_FTE3_hover = sum(isnull(Bucket3_FTEs_hover,0)), @TotalCat_FTE4_hover = sum(isnull(Bucket4_FTEs_hover,0)), @TotalCat_FTE5_hover = sum(isnull(Bucket5_FTEs_hover,0)), @TotalCat_FTE6_hover = sum(isnull(Bucket6_FTEs_hover,0)), @TotalCat_FTE7_hover = sum(isnull(Bucket7_FTEs_hover,0)), @TotalCat_FTE8_hover = sum(isnull(Bucket8_FTEs_hover,0)), @TotalCat_FTE9_hover = sum(isnull(Bucket9_FTEs_hover,0)), @TotalCat_FTE10_hover = sum(isnull(Bucket10_FTEs_hover,0)), @TotalCat_FTE11_hover = sum(isnull(Bucket11_FTEs_hover,0)), @TotalCat_FTE12_hover = sum(isnull(Bucket12_FTEs_hover,0))
				from #TEMP_OUT where RecNumber = 50 and TaskCategoryRecNum50 = @CurTaskCatCode and RecTypeID = @CurAFE_DescID 

				-- Update TASK Category  Rec Type = 40
				update #TEMP_OUT set
					Bucket1_Hours = isnull(@TotalCat_Bucket1,0),Bucket1_FTEs = isnull(@TotalCat_FTE1,0), Bucket1_FTEs_hover = isnull(@TotalCat_FTE1_hover,0),
					Bucket2_Hours = isnull(@TotalCat_Bucket2,0),Bucket2_FTEs = isnull(@TotalCat_FTE2,0), Bucket2_FTEs_hover = isnull(@TotalCat_FTE2_hover,0),
					Bucket3_Hours = isnull(@TotalCat_Bucket3,0),Bucket3_FTEs = isnull(@TotalCat_FTE3,0), Bucket3_FTEs_hover = isnull(@TotalCat_FTE3_hover,0),
					Bucket4_Hours = isnull(@TotalCat_Bucket4,0),Bucket4_FTEs = isnull(@TotalCat_FTE4,0), Bucket4_FTEs_hover = isnull(@TotalCat_FTE4_hover,0),
					Bucket5_Hours = isnull(@TotalCat_Bucket5,0),Bucket5_FTEs = isnull(@TotalCat_FTE5,0), Bucket5_FTEs_hover = isnull(@TotalCat_FTE5_hover,0),
					Bucket6_Hours = isnull(@TotalCat_Bucket6,0),Bucket6_FTEs = isnull(@TotalCat_FTE6,0), Bucket6_FTEs_hover = isnull(@TotalCat_FTE6_hover,0),
					Bucket7_Hours = isnull(@TotalCat_Bucket7,0),Bucket7_FTEs = isnull(@TotalCat_FTE7,0), Bucket7_FTEs_hover = isnull(@TotalCat_FTE7_hover,0),
					Bucket8_Hours = isnull(@TotalCat_Bucket8,0),Bucket8_FTEs = isnull(@TotalCat_FTE8,0), Bucket8_FTEs_hover = isnull(@TotalCat_FTE8_hover,0),
					Bucket9_Hours = isnull(@TotalCat_Bucket9,0),Bucket9_FTEs = isnull(@TotalCat_FTE9,0), Bucket9_FTEs_hover = isnull(@TotalCat_FTE9_hover,0),
					Bucket10_Hours = isnull(@TotalCat_bucket10,0),Bucket10_FTEs = isnull(@TotalCat_FTE10,0), Bucket10_FTEs_hover = isnull(@TotalCat_FTE10_hover,0),
					Bucket11_Hours = isnull(@TotalCat_bucket11,0),Bucket11_FTEs = isnull(@TotalCat_FTE11,0), Bucket11_FTEs_hover = isnull(@TotalCat_FTE11_hover,0),
					Bucket12_Hours = isnull(@TotalCat_bucket12,0),Bucket12_FTEs = isnull(@TotalCat_FTE12,0), Bucket12_FTEs_hover = isnull(@TotalCat_FTE12_hover,0),
					TotalHours = (isnull(@TotalCat_Bucket1,0))+(isnull(@TotalCat_Bucket2,0))+(isnull(@TotalCat_Bucket3,0))+(isnull(@TotalCat_Bucket4,0))+(isnull(@TotalCat_Bucket5,0))+(isnull(@TotalCat_Bucket6,0))+(isnull(@TotalCat_Bucket7,0))+(isnull(@TotalCat_Bucket8,0))+(isnull(@TotalCat_Bucket9,0))+(isnull(@TotalCat_bucket10,0))+(isnull(@TotalCat_bucket11,0))+(isnull(@TotalCat_bucket12,0)),
					TotalFTEs = ((isnull(@TotalCat_FTE1,0))+(isnull(@TotalCat_FTE2,0))+(isnull(@TotalCat_FTE3,0))+(isnull(@TotalCat_FTE4,0))+(isnull(@TotalCat_FTE5,0))+(isnull(@TotalCat_FTE6,0))+(isnull(@TotalCat_FTE7,0))+(isnull(@TotalCat_FTE8,0))+(isnull(@TotalCat_FTE9,0))+(isnull(@TotalCat_FTE10,0))+(isnull(@TotalCat_FTE11,0))+(isnull(@TotalCat_FTE12,0)) ),
					TotalFTEs_hover = ((isnull(@TotalCat_FTE1_hover,0))+(isnull(@TotalCat_FTE2_hover,0))+(isnull(@TotalCat_FTE3_hover,0))+(isnull(@TotalCat_FTE4_hover,0))+(isnull(@TotalCat_FTE5_hover,0))+(isnull(@TotalCat_FTE6_hover,0))+(isnull(@TotalCat_FTE7_hover,0))+(isnull(@TotalCat_FTE8_hover,0))+(isnull(@TotalCat_FTE9_hover,0))+(isnull(@TotalCat_FTE10_hover,0))+(isnull(@TotalCat_FTE11_hover,0))+(isnull(@TotalCat_FTE12_hover,0)) )
				where RecNumber = 40 and RecTypeID = @CurAFE_DescID and    TaskCategory = @CurTaskCatCode      
