...
--select distinct ResourceOrg from #TEMP_IN order by ResourceOrg
----------------------------------------------------------------------------------
...
-- Correct Percentage Summary of Co-Funded Projects for Client Group Report
...
BEGIN
	...
	select @CurProjPct = sum(isnull(ProjClientFundingPct,0)) from #TEMP_PCT where ProjectID = @CurProjectID group by HrsToDate
    ...
    DECLARE Resource_cursor CURSOR FOR
		select distinct ResourceName, JobCode, ResourceOrg, Onshore, Offshore from #TEMP_IN where ProjectID = @CurProjectID order by ResourceName
    OPEN Resource_cursor
    FETCH NEXT FROM Resource_cursor INTO @CurResource, @CurJobCode, @ResourceOrg, @Onshore, @Offshore
    WHILE @@FETCH_STATUS = 0
    BEGIN
		set @SCSC_Hours = 0
        set @SCSC_Billed = 0
		...
		-- If the billing rate from the CO_RESOURCE table is zero and ResourceOrg is not OTHER or RDC, use the Job Code billing rate.
		...
		if @BillingRate = 0	and @ResourceOrg not in ('OTHER','RDC')
			...
			DECLARE JobCodeBillingRate_cursor CURSOR FOR 
				Select BillingRate from lkBillingRateJobCode where EDSJobCode = @CurJobCode 
			OPEN JobCodeBillingRate_cursor
			FETCH NEXT FROM JobCodeBillingRate_cursor INTO @CurJobCodeRate
			WHILE @@FETCH_STATUS = 0
			BEGIN	
				-- Set the billing rate the Job Code billing rate.
		...
		select @SCSC_Billed = isnull(@SCSC_Hours * @BillingRate,0) from #TEMP_IN where ProjectID = @CurProjectID and ResourceName = @CurResource 
			and ResourceOrg = 'SOUTHERN CALIFORNIA SOLUTION CEN'

		select @RDC_Hours = isnull(sum(Hours),0) from #TEMP_IN where ProjectID = @CurProjectID and ResourceName = @CurResource
			and ResourceOrg = 'RDC'
		select @RDC_Billed = isnull(@SCSC_Hours * @BillingRate,0) from #TEMP_IN where ProjectID = @CurProjectID and ResourceName = @CurResource 
			and ResourceOrg = 'RDC'
		...
		select @MEXSC_Billed = isnull(@MEXSC_Hours * @BillingRate,0) from #TEMP_IN where ProjectID = @CurProjectID and ResourceName = @CurResource 
			and (ResourceOrg like '%MEXICO%' or ResourceOrg like '%AD-MX%' ) and ResourceOrg <> 'GCR - MEXICO'
		...
		select @GCR_MEX_Billed = isnull(@GCR_MEX_Hours * @BillingRate,0) from #TEMP_IN where ProjectID = @CurProjectID and ResourceName = @CurResource 
			and ResourceOrg = 'GCR - MEXICO'
		...
		select @MPHASIS_Onsite_Billed = isnull(@MPHASIS_Onsite_Hours * @BillingRate,0) from #TEMP_IN where ProjectID = @CurProjectID and ResourceName = @CurResource 
			and ResourceOrg = 'MPHASIS' and onshore = 1
		...
		select @MPHASIS_Offsite_Billed = isnull(@MPHASIS_Offsite_Hours * @BillingRate,0) from #TEMP_IN where ProjectID = @CurProjectID and ResourceName = @CurResource 
			and ResourceOrg = 'MPHASIS' and offshore = 1
		...
		select @HPINDIA_Billed = isnull(@HPINDIA_Hours * @BillingRate,0) from #TEMP_IN where ProjectID = @CurProjectID and ResourceName = @CurResource 
			and ResourceOrg = 'GCR - SYDNEY'
		...
		select @GCR_SYD_Billed = isnull(@GCR_SYD_Hours * @BillingRate,0) from #TEMP_IN where ProjectID = @CurProjectID and ResourceName = @CurResource 
			and ResourceOrg = 'KOREA'

		select @KOREA_Hours = isnull(sum(Hours),0) from #TEMP_IN where ProjectID = @CurProjectID and ResourceName = @CurResource 
			and ResourceOrg = 'OTHER'
		select @KOREA_Billed = isnull(@KOREA_Hours * @BillingRate,0) from #TEMP_IN where ProjectID = @CurProjectID and ResourceName = @CurResource 
			and ResourceOrg = 'OTHER'
		...
		select @OTHER_Billed = isnull(@OTHER_Hours * @BillingRate,0) from #TEMP_IN where ProjectID = @CurProjectID and ResourceName = @CurResource 
			and ResourceOrg = 'HP INDIA'

        set @TotalHours = @SCSC_Hours +  @RDC_Hours + @MEXSC_Hours + @GCR_MEX_Hours + @MPHASIS_Onsite_Hours + @MPHASIS_Offsite_Hours + @HPINDIA_Hours + @GCR_SYD_Hours + @KOREA_Hours + @OTHER_Hours
		set @TotalBilled = @SCSC_Billed + @RDC_Billed + @MEXSC_Billed + @GCR_MEX_Billed + @MPHASIS_Onsite_Billed + @MPHASIS_Offsite_Billed + @HPINDIA_Billed + @GCR_SYD_Billed + @KOREA_Billed + @OTHER_Billed
        ...
        select 50, 'Resource', @CurResource, @BillingRate,  @CurProjPct, @TotalHours, @TotalBilled, @SCSC_Hours, @SCSC_Billed, @RDC_Hours, @RDC_Billed, @MEXSC_Hours, @MEXSC_Billed, @GCR_MEX_Hours, @GCR_MEX_Billed, @MPHASIS_Onsite_Hours, @MPHASIS_Onsite_Billed, @MPHASIS_Offsite_Hours, @MPHASIS_Offsite_Billed, @HPINDIA_Hours, @HPINDIA_Billed, @GCR_SYD_Hours, @GCR_SYD_Billed, @KOREA_Hours, @KOREA_Billed, @OTHER_Hours, @OTHER_Billed, @ClientGroup, @CurProjectTitle
...
update #TEMP_OUT set SCSC_Hours = @SCSC_Hours/@FTE_Factor_SCSC, SCSC_Billed = @SCSC_Billed/@FTE_Factor_SCSC, RDC_Hours = @RDC_Hours/@FTE_Factor_RDC, RDC_Billed = @RDC_Billed/@FTE_Factor_RDC, MEXSC_Hours = @MEXSC_Hours/@FTE_Factor_MEXSC, MEXSC_Billed = @MEXSC_Billed/@FTE_Factor_MEXSC, GCR_MEX_Hours = @GCR_MEX_Hours/@FTE_Factor_MEXSC, GCR_MEX_Billed = @GCR_MEX_Billed/@FTE_Factor_MEXSC, MPHASIS_Onsite_Hours = @MPHASIS_Onsite_Hours/@FTE_Factor_INDIA, MPHASIS_Onsite_Billed = @MPHASIS_Onsite_Billed/@FTE_Factor_INDIA,MPHASIS_Offsite_Hours = @MPHASIS_Offsite_Hours/@FTE_Factor_INDIA, MPHASIS_Offsite_Billed = @MPHASIS_Offsite_Billed/@FTE_Factor_INDIA, HPINDIA_Hours = @HPINDIA_Hours/@FTE_Factor_GCR_SYDNEY, HPINDIA_Billed = @HPINDIA_Billed/@FTE_Factor_GCR_SYDNEY, GCR_SYD_Hours = @GCR_SYD_Hours/@FTE_Factor_KOREA, GCR_SYD_Billed = @GCR_SYD_Billed/@FTE_Factor_KOREA, KOREA_Hours = @KOREA_Hours/@FTE_Factor_OTHER, KOREA_Billed = @KOREA_Billed/@FTE_Factor_OTHER, OTHER_Hours = @OTHER_Hours/@FTE_Factor_INDIA, OTHER_Billed = @OTHER_Billed/@FTE_Factor_INDIA,
TotalHours = @SCSC_Hours/@FTE_Factor_SCSC + @RDC_Hours/@FTE_Factor_RDC + @MEXSC_Hours/@FTE_Factor_MEXSC + @GCR_MEX_Hours/@FTE_Factor_MEXSC + @MPHASIS_Onsite_Hours/@FTE_Factor_INDIA + @MPHASIS_Offsite_Hours/@FTE_Factor_INDIA + @HPINDIA_Hours/@FTE_Factor_GCR_SYDNEY + @GCR_SYD_Hours/@FTE_Factor_KOREA + @KOREA_Hours/@FTE_Factor_OTHER + @OTHER_Hours/@FTE_Factor_INDIA,
TotalBilled = @SCSC_Billed/@FTE_Factor_SCSC + @RDC_Billed/@FTE_Factor_RDC + @MEXSC_Billed/@FTE_Factor_MEXSC + @GCR_MEX_Billed/@FTE_Factor_MEXSC + @MPHASIS_Onsite_Billed/@FTE_Factor_INDIA + @MPHASIS_Offsite_Billed/@FTE_Factor_INDIA + @HPINDIA_Billed/@FTE_Factor_GCR_SYDNEY + @GCR_SYD_Billed/@FTE_Factor_KOREA + @KOREA_Billed/@FTE_Factor_OTHER + @OTHER_Billed/@FTE_Factor_INDIA
where RecNumber = 5

Update #TEMP_OUT set SCSC_Hours = @FTE_Factor_SCSC, RDC_Hours = @FTE_Factor_RDC, MEXSC_Hours = @FTE_Factor_MEXSC, GCR_MEX_Hours = @FTE_Factor_MEXSC, MPHASIS_Onsite_Hours = @FTE_Factor_INDIA, MPHASIS_Offsite_Hours = @FTE_Factor_INDIA, HPINDIA_Hours = @FTE_Factor_GCR_SYDNEY, GCR_SYD_Hours = @FTE_Factor_KOREA, KOREA_Hours = @FTE_Factor_OTHER, OTHER_Hours = @FTE_Factor_INDIA
where RecNumber = 7
...
Select RecNumber, RecType, RecDesc, HourlyBillingRate, FundingPct, TotalHours, TotalBilled, SCSC_Hours, SCSC_Billed, RDC_Hours, RDC_Billed, MEXSC_Hours, MEXSC_Billed, GCR_MEX_Hours, GCR_MEX_Billed, MPHASIS_Onsite_Hours, MPHASIS_Onsite_Billed, MPHASIS_Offsite_Hours, MPHASIS_Offsite_Billed, OTHER_Hours, OTHER_Billed, HPINDIA_Hours, HPINDIA_Billed, GCR_SYD_Hours, GCR_SYD_Billed, KOREA_Hours, KOREA_Billed, R10_Client, R30_Project
